...
@util.subprocess
def gpu(gsc, exc, vel, dt, dataset, nsv, tf=1500, ds=80, model="fhn_euler", cvar=0,
...
def launches(datasets, models, vel, G, E, nic, dt, blockDim_x=256, gridDim_x=256):
    ...
    for d, m, v in itertools.product(datasets, models, vel):
        ...
        if G_.size <= nthr:
            ...
            for l in range(G_.size/nthr-1):
                yield G_[  l   *nthr :(l+1)*nthr ], E_[  l   *nthr :(l+1)*nthr ], v
            if G_.size - G_.size/nthr*nthr > 0:
                yield G_[ (l+1)*nthr :           ], E_[ (l+1)*nthr :           ], v


def reducer(Xs):
    Xs = Xs[:, -Xs.shape[1]/2:].reshape((G.size, -1, n*nsv)).copy() # copy to gc original
...
if __name__ == '__main__':
    ...
    for launch in launches(datasets, models, vel, gsc, exc, nic, dt):
        temp = concatenate((temp.copy(), gpu(launch)))
        if temp.shape[0] == G.size*nic:
            for r in consumer(temp, nic):
                print r
